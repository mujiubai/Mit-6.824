...
type ShardKV struct {
	...
	dead         int32
	replayFinish bool //用于判断服务器新建后以前的日志是否重放完成，重放完成才开始检测新配置
...
func (kv *ShardKV) applyLoop() {
	...
	for message := range kv.applyCh {
		...
		if op.Operation == Put || op.Operation == Get || op.Operation == Append {
			...
			case FlagEntry:
				kv.DPrintf("kv[%v %v] detect FlagEntry, replay entries finish!!", kv.gid, kv.me)
				kv.replayFinish = true
			}
			res.Err = OK
		}
...
func (kv *ShardKV) detecConfLoop() {
	for !kv.killed() {
		...
		kv.mu.Lock()
		if !kv.replayFinish {
			kv.DPrintf("kv[%v %v] replay previous log is not finish!!", kv.gid, kv.me)
			kv.mu.Unlock()
...
func (kv *ShardKV) genSnapData() []byte {
	...
	kv.DPrintf("kv[%v %v] generate snadata success, kvdb=%v", kv.gid, kv.me, kv.kvDB)
	return w.Bytes()
}
...
func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int, gid int, ctrlers []*labrpc.ClientEnd, make_end func(string) *labrpc.ClientEnd) *ShardKV {
	...
	//需要提交一条FlagEntry日志，用于判断持久化的日志是否重放完成，重放完成才能开始检测新配置或发送shard数据
	go func() {
		time.Sleep(time.Duration(50) * time.Millisecond)
		op := Op{}
		op.Operation = FlagEntry
		for {
			kv.mu.Lock()
			index, term, isLeader := kv.rf.Start(op)
